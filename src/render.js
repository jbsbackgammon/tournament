...
export function renderBracket(state, options = {}) {
  ...
  const line = size === 64 ? 8 : size === 32 ? 11 : 14;
  ...
  const supplementText = (value, x, y, maxWidth, anchor = 'middle') => {
    ...
    const needsWrap = chars.length > 7;
  ...
  if (state.showMatchNotes || state.showResultNotes) {
    for (let r = 0; r < levels - 1; r++) for (let i = 0; i < nodes[r].length; i += 2) {
      ...
      if (state.showResultNotes) {
        ...
        const x = p.side ? p.x - 2 : p.x + 10;
        supplementParts.push(supplementText(upperResult, x, nodes[r][i].y + font * .2, Math.abs(dest.x - p.x) - 20, align));
        supplementParts.push(supplementText(lowerResult, x, nodes[r][i + 1].y + font * .2, Math.abs(dest.x - p.x) - 20, align));
      }
    }
  }
